"""Coordinator for the Slovenská Pošta parcel tracker integration.
...
from .api import SlovenskaPostaApiClient, SlovenskaPostaApiError
from .const import (
...
class SlovenskaPostaCoordinator(DataUpdateCoordinator[list[dict]]):
    ...
    refresh fetches them in one batched ``q=`` call (see
    :meth:`SlovenskaPostaApiClient.async_get_parcels`) rather than one request
    per code — that's the point of the documented comma-list.
    ...
    def __init__(
        self,
        hass: HomeAssistant,
        client: SlovenskaPostaApiClient,
        entry: ConfigEntry,
    ) -> None:
        ...
        # tracking_code -> last successful raw result item, so a transient
        # fetch failure keeps the parcel visible instead of dropping its
        # sensor. Lives for the integration's lifetime (resets on restart).
        # Not used for a genuine "not found"/empty-history reply — that's a
        # fresh, real signal, not a failure to paper over.
    ...
    async def _async_update_data(self) -> list[dict]:
        ...
        try:
            ...
            if isinstance(err, SlovenskaPostaApiError) and err.status_code == 429:
                # A 429 means the whole poll backs off — one hot parcel's
                # cadence must not keep hammering an endpoint that just asked
                ...
                raise UpdateFailed(
                    "Slovenská Pošta rate-limited (429)", retry_after=retry_after
                ) from err
            _LOGGER.warning("Slovenská Pošta batch fetch failed: %s", err)
        else:
            self._consecutive_429 = 0

        raws_by_code: dict[str, dict] = {}
        if fetched is None:
            # The whole batched request failed outright — fall back to
            # whatever cached data exists, no synthetic pending placeholders
            # (that would misrepresent an outage as "not yet trackable").
            raws_by_code = {
                code: self._raw_cache[code]
                for code in codes_to_fetch
                if code in self._raw_cache
            }
            errors = len(codes_to_fetch)
        else:
            errors = 0
            for code in codes_to_fetch:
                result = fetched.get(code)
                if result is None:
                    # A genuine "no history yet"/unknown code (or one the
                    # carrier rejected as invalid_format, already filtered out
                    # by the client) — keep prior data if we have it,
                    # otherwise show a pending placeholder so the user still
                    # sees the parcel they asked us to track.
                    raws_by_code[code] = self._raw_cache.get(code) or {"number": code}
                    continue
        ...
        if codes_to_fetch and errors == len(codes_to_fetch) and not raws_by_code:
            raise UpdateFailed("Slovenská Pošta unreachable for all tracked parcels")
