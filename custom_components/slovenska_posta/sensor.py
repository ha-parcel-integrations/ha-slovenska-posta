"""Sensor platform for the Slovenská Pošta parcel tracker integration."""
...
from .coordinator import SlovenskaPostaCoordinator
...
async def async_setup_entry(
    hass: HomeAssistant,
    entry: SlovenskaPostaConfigEntry,
    async_add_entities: AddEntitiesCallback,
) -> None:
    """Set up Slovenská Pošta sensor entities from a config entry."""
    ...
    entities: list[SensorEntity] = [
        SlovenskaPostaIncomingParcelsSensor(
            coordinator, entry, async_add_entities, current_barcodes
        ),
    ]
    for parcel in coordinator.data or []:
        entities.append(
            SlovenskaPostaParcelSensor(coordinator, entry, parcel.get("barcode", ""))
        )
    entities.append(SlovenskaPostaAwaitingPickupSensor(coordinator, entry))
    entities.append(SlovenskaPostaNextDeliverySensor(coordinator, entry))
    entities.append(SlovenskaPostaDeliveredParcelsSensor(coordinator, entry))
    entities.append(SlovenskaPostaLastUpdateSensor(coordinator, entry))

    async_add_entities(entities)


class SlovenskaPostaIncomingParcelsSensor(
    CoordinatorEntity[SlovenskaPostaCoordinator], SensorEntity
):
    ...
    def __init__(
        self,
        coordinator: SlovenskaPostaCoordinator,
        entry: ConfigEntry,
        async_add_entities: AddEntitiesCallback,
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                SlovenskaPostaParcelSensor(self.coordinator, self._entry, barcode)
...
class SlovenskaPostaAwaitingPickupSensor(
    CoordinatorEntity[SlovenskaPostaCoordinator], SensorEntity
):
    """Parcels waiting for collection at a post office right now.

    Reachable via the carrier's own ``notified`` state (mapped to
    ``AT_PICKUP_POINT``), so this carrier gets the suite's standard
    ...
    def __init__(
        self, coordinator: SlovenskaPostaCoordinator, entry: ConfigEntry
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class SlovenskaPostaParcelSensor(CoordinatorEntity[SlovenskaPostaCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked Slovenská Pošta parcel."""
    ...
    def __init__(
        self, coordinator: SlovenskaPostaCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._entry = entry
...
class SlovenskaPostaNextDeliverySensor(
    CoordinatorEntity[SlovenskaPostaCoordinator], SensorEntity
):
...
class SlovenskaPostaDeliveredParcelsSensor(
    CoordinatorEntity[SlovenskaPostaCoordinator], SensorEntity
):
    """Recently delivered tracked Slovenská Pošta parcels."""
...
class SlovenskaPostaLastUpdateSensor(
    CoordinatorEntity[SlovenskaPostaCoordinator], SensorEntity
):
    """Diagnostic sensor reporting when Slovenská Pošta was last polled successfully."""
