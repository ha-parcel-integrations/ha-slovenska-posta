"""Services for the Slovenská Pošta parcel tracker integration.

`slovenska_posta.track_parcel` / `slovenska_posta.untrack_parcel` let you add or remove a
...
def _resolve_entry(hass: HomeAssistant):
    """Return the single Slovenská Pošta hub, or raise when it is not set up."""
    entries = hass.config_entries.async_entries(DOMAIN)
    if not entries:
        raise ServiceValidationError("Slovenská Pošta is not set up")
    return entries[0]


def async_setup_services(hass: HomeAssistant) -> None:
    """Register the Slovenská Pošta services (idempotent)."""
    ...
    async def _track(call: ServiceCall) -> None:
        ...
        if not valid_tracking_code(tracking_code):
            raise ServiceValidationError(
                f"'{tracking_code}' is not a valid Slovenská Pošta tracking code"
            )
...
def async_unload_services(hass: HomeAssistant) -> None:
    """Remove the Slovenská Pošta services (single-entry integration, so on unload)."""
