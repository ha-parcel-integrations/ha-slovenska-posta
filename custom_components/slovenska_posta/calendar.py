"""Calendar platform for the Slovenská Pošta parcel tracker integration."""
...
async def async_setup_entry(
    ...
    """Set up the Slovenská Pošta deliveries calendar from a config entry."""
    async_add_entities([SlovenskaPostaDeliveriesCalendar(entry.runtime_data.coordinator, entry)])


class SlovenskaPostaDeliveriesCalendar(CoordinatorEntity[SlovenskaPostaCoordinator], CalendarEntity):
    """A read-only calendar of expected Slovenská Pošta deliveries.
    ...
    def __init__(self, coordinator: SlovenskaPostaCoordinator, entry: ConfigEntry) -> None:
    ...
    def _events(self) -> list[CalendarEvent]:
        ...
        for parcel in self.coordinator.data or []:
            ...
            summary = sender or (f"Parcel {barcode}" if barcode else "Slovenská Pošta parcel")
            description_parts = [
