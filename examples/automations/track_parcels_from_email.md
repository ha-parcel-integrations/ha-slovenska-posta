# Automatic parcel tracking from e-mail (IMAP → Slovenská Pošta)

Companion guide for [`track_parcels_from_email.yaml`](track_parcels_from_email.yaml): watch your mailbox(es) for shipping e-mails, extract the Slovenská Pošta tracking code, and register it with `slovenska_posta.track_parcel` — fully automatic, no extra custom component required.

Slovenská Pošta is a **code-based** carrier: it has no account inbox, so every parcel must be registered by its tracking code before the integration can follow it. This recipe automates exactly that step.

**How it works, in one line:** the core [IMAP integration](https://www.home-assistant.io/integrations/imap/) fires an `imap_content` event for every new e-mail (including the body); the automation extracts the tracking code — a cheap regex first, an optional AI fallback for everything else — and calls `slovenska_posta.track_parcel`.

```
new e-mail ──imap_content──▶ automation ──▶ regex match? ──▶ slovenska_posta.track_parcel
                                     │
...
- This integration, with the `slovenska_posta.track_parcel` action available (field `tracking_code`).
...
Slovenská Pošta's own manual doesn't document a tracking-code shape, so the
regex matches the general UPU "S10" shape most European postal tracking
codes use instead of a confirmed carrier prefix — 2 letters, 9 digits, then
a 1-2 letter country code (e.g. `RR123456789SK`):

```
(?<![0-9A-Za-z])[A-Za-z]{2}[0-9]{9}[A-Za-z]{1,2}(?![0-9A-Za-z])
```

The integration itself is deliberately more permissive (any 8-20 character
code without control characters, the carrier's live lookup is the actual
authority — see the main README), so a code that doesn't fit the S10 shape
above will still track fine if entered manually; it just won't be picked up
by this regex. **If your mails carry a different shape, adjust the pattern**;
the AI fallback will surface unknown formats in the meantime.
...
curl -X POST -H "Authorization: Bearer $HA_TOKEN" -H "Content-Type: application/json" \
  ...
  -d '{"sender":"noreply@posta.sk","subject":"Your parcel is on its way",
       "text":"Tracking code: RR123456789SK","initial":true,"folder":"INBOX","username":"test"}'
```

Then `slovenska_posta.untrack_parcel` the test code afterwards. For a full end-to-end test, forward a real shipping mail to the watched mailbox — it must arrive **unread**.
