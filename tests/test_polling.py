...
from custom_components.slovenska_posta.coordinator import (
    SlovenskaPostaCoordinator,
...
async def test_update_interval_resumes_once_a_parcel_is_added(hass):
    """Slovenská Pošta's status map never reaches OUT_FOR_DELIVERY (Trap-adjacent:
    no such carrier state), so a tracked, in-transit parcel resumes at the
    mid tier, not hot — hot is covered by the pure-function tests above.
    """
    entry = _entry_with([])
    entry.add_to_hass(hass)
    client = AsyncMock()
...
async def test_429_raises_update_failed_with_retry_after(hass):
    ...
    entry.add_to_hass(hass)
    client = AsyncMock()
    client.async_get_parcels.side_effect = SlovenskaPostaApiError(
