"""Tests for Slovenská Pošta setup and unload."""
...
OTHER_CODE = "RR000000008SK"


def _batch_mock(sample=None):
    """A batch mock keyed by whichever codes are actually requested."""

    async def _get(codes):
        return {code: (sample or _sample)(code) for code in codes}
...
async def test_setup_and_unload(hass):
    ...
    incoming = hass.states.get("sensor.slovenska_posta_incoming_parcels")
...
async def test_setup_retries_when_first_refresh_fails(hass):
    ...
    with patch(
        "custom_components.slovenska_posta.api.SlovenskaPostaApiClient.async_get_parcels",
        new=AsyncMock(side_effect=SlovenskaPostaApiError("Slovenská Pošta unreachable")),
    ):
