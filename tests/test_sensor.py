"""Tests for Slovenská Pošta sensor property logic."""
from datetime import datetime, timezone
from unittest.mock import MagicMock

from custom_components.slovenska_posta.const import ParcelStatus
from custom_components.slovenska_posta.sensor import (
    SlovenskaPostaAwaitingPickupSensor,
    SlovenskaPostaDeliveredParcelsSensor,
    SlovenskaPostaIncomingParcelsSensor,
    SlovenskaPostaLastUpdateSensor,
    SlovenskaPostaNextDeliverySensor,
    SlovenskaPostaParcelSensor,
)
...
def test_incoming_counts_and_lists():
    ...
    sensor = SlovenskaPostaIncomingParcelsSensor(coordinator, _entry(), lambda _: None, set())
...
def test_parcel_sensor_status_and_attributes():
    ...
    sensor = SlovenskaPostaParcelSensor(_coordinator([parcel]), _entry(), "A")
...
def test_parcel_sensor_missing_barcode():
    sensor = SlovenskaPostaParcelSensor(_coordinator([_parcel("A")]), _entry(), "OTHER")
    assert sensor.native_value is None
    assert sensor.extra_state_attributes == {}


def test_awaiting_pickup_counts_only_at_pickup_point():
    ...
    sensor = SlovenskaPostaAwaitingPickupSensor(coordinator, _entry())
    assert sensor.native_value == 2
...
def test_next_delivery_picks_earliest():
    ...
    sensor = SlovenskaPostaNextDeliverySensor(coordinator, _entry())
...
def test_next_delivery_none_without_moments():
    sensor = SlovenskaPostaNextDeliverySensor(_coordinator([_parcel("A")]), _entry())
    assert sensor.native_value is None
    assert sensor.extra_state_attributes == {}
...
def test_delivered_sensor():
    ...
    sensor = SlovenskaPostaDeliveredParcelsSensor(coordinator, _entry())
...
def test_last_update_sensor():
    ...
    sensor = SlovenskaPostaLastUpdateSensor(coordinator, _entry())
